...
def add_patient():
    ...
    # Call OTHER function to dp the request (no test on server)
...
def validate_patient_id(patient_id):
    try:
        ...
        return "Patient_id was not an interger", 400
    return True, 200
...
def add_patient_driver(in_data):
    ...
    if status_code != 200:
        return answer, status_code
    db = add_patient_to_db(
        in_data["name"], in_data["id"], in_data["blood_type"])
    print(db)
    return True, 200
